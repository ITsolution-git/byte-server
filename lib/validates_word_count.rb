  ...
  module Validations
    module ClassMethods
      ...
      def validates_word_count(*args)
        ...
        if configuration[:in] && configuration[:in].is_a?(Range)
          minimum = configuration[:in].min
          maximum = configuration[:in].max
        else
          if configuration[:minimum] && configuration[:minimum].is_a?(Fixnum)
            minimum = configuration[:minimum].to_i
          end

          if configuration[:maximum] && configuration[:maximum].is_a?(Fixnum)
        ...
        validates_each(args, configuration) do |record, attr_name, value|
          ...
          if configuration[:item_name].is_a?(Symbol) && record.respond_to?(configuration[:item_name])
          ...
          if !maximum.nil? && (word_count > maximum)
          ...
          elsif !minimum.nil? && (word_count < minimum)
